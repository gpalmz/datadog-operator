...
type AdmissionControllerProbeConfig struct {
	...
	// The probe periodically sends dry-run pod creation requests to verify the webhook
	// is reachable from the API server. Requires Cluster Agent 7.78.0+.
	// Default: true
	// +optional
	Enabled *bool `json:"enabled,omitempty"`

	// Namespace is the Kubernetes namespace used for dry-run probe pods.
	// The namespace is created automatically when the probe is enabled.
	// Default: "datadog-admission-probe-dryrun"
	// +optional
