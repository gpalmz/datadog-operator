...
func Test_admissionControllerFeature_Configure(t *testing.T) {
	...
	tests := test.FeatureTestSuite{
		...
		{
			Name: "Admission Controller enabled with custom probe namespace",
			DDA: testutils.NewDatadogAgentBuilder().
				WithAdmissionControllerEnabled(true).
				WithAdmissionControllerProbeNamespace("custom-probe-ns").
				Build(),
			WantConfigure: true,
			ClusterAgent: test.NewDefaultComponentTest().WithWantFunc(
				probeCustomNamespaceWantFunc("custom-probe-ns")),
		},
	}
...
func testDCAResources(acm string, registry string, cwsInstrumentationEnabled bool) *test.ComponentTest {
	return test.NewDefaultComponentTest().WithWantFunc(
		func(t testing.TB, mgrInterface feature.PodTemplateManagers) {
			...
			expectedAgentEnvs = append(expectedAgentEnvs, getProbeEnvVars(defaultProbeNamespace)...)
...
func getACEnvVars(validation, mutation bool, acm, registry string, cws bool) []*corev1.EnvVar {
	...
	envVars = append(envVars, getProbeEnvVars(defaultProbeNamespace)...)

	return envVars
}

func getProbeEnvVars(namespace string) []*corev1.EnvVar {
...
func probeCustomNamespaceWantFunc(namespace string) func(testing.TB, feature.PodTemplateManagers) {
	return func(t testing.TB, mgrInterface feature.PodTemplateManagers) {
		mgr := mgrInterface.(*fake.PodTemplateManagers)
		dcaEnvVars := mgr.EnvVarMgr.EnvVarsByC[apicommon.ClusterAgentContainerName]
		want := getACEnvVarsNoProbe(false, false, "", "", false)
		want = append(want, getProbeEnvVars(namespace)...)
		assert.ElementsMatch(
			t,
			dcaEnvVars,
			want,
